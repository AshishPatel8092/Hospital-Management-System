...
router.post('/register', registerLimiter, async (req, res) => {
  ...
  try {
    ...
    // email_verified is set straight to 1 - this project has no real email
    // service configured, so gating login on a verification email would
    // just lock people out with no way to receive the code. Everything
    // else (rate limiting, strong passwords, account lockout) still applies.
    ...
    const user = { user_id: userId, full_name: fullName, email, role: upperRole };
    ...
    establishSession(req, user, linkedId, (err, sessionUser) => {
      ...
      res.status(201).json({ success: true, message: 'Account created.', data: sessionUser });
    });
  } catch (err) {
