  ...
  try {
    if (req.query.id) {
      const [rows] = await pool.execute(
        `SELECT d.*, u.email, u.phone FROM doctors d
         JOIN users u ON u.user_id = d.user_id WHERE d.doctor_id = ?`,
        [req.query.id]
      );
    ...
    let sql = `SELECT d.*, u.email, u.phone FROM doctors d JOIN users u ON u.user_id = d.user_id`;
    const params = [];
